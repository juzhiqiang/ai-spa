...
import { formatAddress, generateGradientAvatar } from '../utils/avatarGenerator';
...
const WalletConnection: React.FC<WalletConnectionProps> = ({
    ...
    const getDisplayName = (
        address: string,
        ensData?: { name: string | null; isLoading: boolean },
    ) => {
        if (ensData?.isLoading) {
            return (
                <div
            ...
            {account ? (
                <div className="wallet-menu-container" style={{ position: 'relative' }}>
                    <div
                        ...
                        onClick={() =>
                            !isDisconnecting && !isSwitching && setShowWalletMenu(!showWalletMenu)
                        }
                        style={{
                            display: 'flex',
                            alignItems: 'center',
                    ...
                    >
                        <div style={{ width: '32px', height: '32px', position: 'relative' }}>
                            {renderAvatar(account, 32, {
                                name: mainEnsName,
                                avatar: mainEnsAvatar,
                                isLoading: mainEnsLoading,
                            })}
                        </div>
                ...
                >
                    ...
                    {isConnecting
                        ? TEXT?.CONNECTING || '连接中...'
                        : TEXT?.CONNECT_WALLET || '连接 MetaMask'}
                </button>
            )}
        </div>
    );

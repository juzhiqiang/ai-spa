...
export const useContract = (provider: BrowserProvider | null) => {
    ...
    const getRedPacketInfo = async (): Promise<RedPacketInfo | null> => {
        try {
            ...
            // 获取基本红包信息
            const [remainingAmount, claimedCount, maxRecipients, isFinished] =
                await contract.getRedPacketInfo();
            ...
            // 计算已分配金额
            const distributedAmount = formatEther(
                BigInt(totalAmount.toString()) - BigInt(remainingAmount.toString()),
            );
